...
description: OpenClaw 持久记忆系统，提供跨会话的长期记忆存储、搜索和上下文继承功能。适用于：(1) 需要记住用户偏好和重要信息 (2) 需要在会话间继承工作进度 (3) 需要搜索历史记忆 (4) 需要构建个人知识库。触发条件：用户提及"记忆"、"记住"、"继续上次"、"搜索记忆"、"知识库"等关键词，或需要保存重要信息时。
---

# Persistent Memory v0.3.0 - 持久记忆系统
...
**触发条件**：上下文达到 `triggerThreshold`（默认 75%）时自动保存
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| triggerThreshold | 0.75 | 触发自动保存的上下文阈值 |
...
- **v0.3.0** (2026-03-15): 正式发布版本
  - 完整功能实现
  - 多语言 README
  - 存储路径优化

- **v2.3** (2026-03-15): 高级优化版
  - 添加增量快照（节省存储 + 更快）
  - 添加自动备份（每日备份 + 7天保留）
  - 添加文件监听（自动保存工作进度）
  - 添加主动确认学习（提高知识质量）
  - 添加多项目管理支持
  - 新增 CLI 命令：backup, backups, restore, watch, unwatch, pending, confirm

- **v2.2** (2026-03-15): 优化增强版
  - 添加时间衰减置信度（30天半衰期）
  - 添加多版本快照（保留最近5个版本）
  - 添加语义截断（按句子截断）
  - 添加重要度等级（1-5星）
  - 添加快照回滚功能
  - 新增 CLI 命令：versions, rollback, events, top, apply-decay

- **v2.1** (2026-03-15): 补充缺失功能
  - 添加置信度追踪（根据使用频率调整知识权重）
  - 添加经验提取（从对话中自动提取知识）
  - 添加趋势分析（分析对话主题和情感）
  - 添加自动学习（autoLearn）
  - 添加低置信度知识清理（pruneLowConfidence）

- **v2.0** (2026-03-15): 完整优化版
  - 添加向量搜索
  - 添加分片存储
  - 添加 LRU 缓存
  - 添加智能摘要
